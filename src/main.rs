...
fn main() {
    ...
    match &cli.command {
        ...
        Commands::ProveJolt(args) => {
            ...
            copy_dir_all(&args.guest_path, "./guest/").unwrap();
            prepend_to_file("./guest/src/main.rs",
            ...
            let (output, proof) = prove_fib();
            let is_valid = verify_fib(proof);

            println!("output: {}", output);
            println!("valid: {}", is_valid);

            let elf_canonical_path = fs::canonicalize("./riscv32im-succinct-zkvm-elf").expect("Failed to canonicalize ELF path");

            // Ensure that the directory exists or create it if it doesn't
            if !elf_canonical_path.exists() {
                if let Err(err) = fs::create_dir_all(&elf_canonical_path) {
...
fn link_toolchain() -> Result<(), Error> {
    let link_path = "rust/build/host/stage2";
...
[dependencies]
jolt = { package = "jolt-sdk", git = "https://github.com/a16z/jolt", version = "0.1.0"  }
"#;